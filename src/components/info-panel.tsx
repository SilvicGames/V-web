...
const InfoBlock = ({ title, children }: { title: string, children: React.ReactNode }) => (
    <div className="flex flex-col items-center gap-1 w-full">
        <p className="font-semibold uppercase tracking-wider text-[11px] opacity-80 text-shadow-sm text-center">{title}</p>
        {children}
    </div>
);
...
const InfoBroom = ({ children }: { children: React.ReactNode }) => (
    <div className="bg-black/10 rounded-lg p-1 border border-black/20 flex justify-center items-center text-center h-28 w-full">
...
export function InfoPanel({ playerScore, opponentScore, previousTableSum, tableSum, hintCards }: {
  ...
  return (
    <div className="flex flex-col w-full h-full justify-between gap-2 rounded-lg bg-secondary/40 px-7 py-2 shadow-inner text-foreground font-body">
